...
alldata = dict()
alldata['1']=dict(nomor=1, nama="Park Seo Joon", posisi="Main Role")
alldata['2']=dict(nomor=2, nama="Park Hyung Sik", posisi="Main Role")
alldata['3']=dict(nomor=3, nama="Go Ah Ra", posisi="Main Role")
alldata['4']=dict(nomor=4, nama="Seo Yea Ji", posisi="Support Role")
alldata['5']=dict(nomor=5, nama="Do Ji Han", posisi="Support Role")
alldata['6']=dict(nomor=6, nama="Choi Min Ho", posisi="Support Role")
alldata['7']=dict(nomor=7, nama="Kim Tae Hyung", posisi="Support Role")
alldata['8']=dict(nomor=8, nama="Jo Yoon Woo", posisi="Support Role")
alldata['9']=dict(nomor=9, nama="Keon Hee", posisi="Guest Role")
alldata['10']=dict(nomor=10, nama="Kim Min Joon", posisi="Guest Role")
...
def handle_request(client_address, connection):
    selesai=False
...
def run_server(server_address, is_secure=False, soal=1):
    ...
    while True:
        ...
        try:
            ...
            if soal == 1:
                handle_request(client_address, connection)
            else:
                threads[thread_index] = threading.Thread(target=handle_request, args=(client_address, connection))
...
if __name__=='__main__':
    try:
        ...
        # run_server(('0.0.0.0', 12000), is_secure=False, soal=1)
        
        # No 2 - Client dan server dengan multi thread dan tidak secure
        # run_server(('0.0.0.0', 12000), is_secure=False, soal=2)
        
        # No 3 - Client dan server dengan multi thread dan mode secure
        # run_server(('0.0.0.0', 12000), is_secure=True, soal=3)
